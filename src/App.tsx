...
import { VerifyReviewSection } from './components/sections/VerifyReviewSection';
...
const NAV_ITEMS = [
  ...
  { id: 'present', label: 'Present 🎁', icon: Gift, color: 'text-amber-500 font-extrabold animate-pulse' },
...
export default function App() {
  ...
  useEffect(() => {
    fpPromise
      ...
      .catch(err => console.error("Error loading FingerprintJS:", err));

    // Support both ref and referrer query params in search & hash
    const checkReferral = () => {
      const searchParams = new URLSearchParams(window.location.search);
      let ref = searchParams.get('ref') || searchParams.get('referrer');
      
      if (!ref) {
        const hashPart = window.location.hash;
        if (hashPart.includes('?')) {
          const queryOfHash = hashPart.split('?')[1];
          const hashParams = new URLSearchParams(queryOfHash);
      ...
      if (ref) {
        ref = ref.trim();
        localStorage.setItem('mdzpx_referrer', ref);
        console.log('Recorded referrer:', ref);
      }
    };
    checkReferral();
  }, []);

  // Trigger automatic anti-cheat loop for multi-account abuse detection in 24/7 background mode
  useEffect(() => {
    if (isLoggedIn && username && fingerprint) {
      syncService.autoBanIfCheating(username, fingerprint);
    }
  }, [isLoggedIn, username, fingerprint]);
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      if (hash && NAV_ITEMS.some(item => item.id === hash)) {
        setActiveTab(hash);
      } else if (hash.startsWith('verify-review')) {
        setActiveTab('verify-review');
      } else if (hash === 'verify') {
        setActiveTab('verify');
      }
    };
  ...
  const handleAuth = async () => {
    ...
    if (authMode === 'register' && !agreeToTerms) {
      setAuthError('Bạn phải đồng ý với điều khoản website để đăng ký');
      return;
    }
    ...
    if (authMode === 'register') {
      ...
      // Handle transparent referral program enrollment with referrer validation
      const refName = localStorage.getItem('mdzpx_referrer');
      if (refName) {
        const trimmedRef = refName.trim();
        let actualReferrer: string | null = null;
        
        // Try resolving as a 5-digit random code first
        actualReferrer = await syncService.resolveReferralCode(trimmedRef);
        
        // If not resolved, treat as fallback username directly
        if (!actualReferrer) {
          const profileDirect = await syncService.getProfile(trimmedRef);
          if (profileDirect) {
            actualReferrer = trimmedRef;
          }
        }
        
        if (actualReferrer && actualReferrer.toLowerCase() !== trimmedUsername.toLowerCase()) {
          const refProfile = await syncService.getProfile(actualReferrer);
          if (refProfile) {
            await syncService.recordReferral(trimmedUsername, actualReferrer);
            console.log(`Registered referral: referee ${trimmedUsername} referred by ${actualReferrer}`);
          }
        }
      }
...
Vui lòng chờ Admin kiểm tra và duyệt đơn. Thẻ Garena sẽ xem trong Lịch Sử Giao Dịch!`;
    ...
    const infoContent = `HỆ THỐNG MOD MDZPX\n------------------\nVẬT PHẨM: ${itemName}\nNGƯỜI DÙNG: ${userNick}\nTRẠNG THÁI: HOẠT ĐỘNG\nNGÀY SỞ HỮU: ${new Date().toLocaleString('vi-VN')}\n\nCảm ơn bạn đã tin dùng dịch vụ của MDZPX. Chúc bạn chơi game vui vẻ!`;

    // Instead of closeModal, change modal to loading state
    setModal({
      isOpen: true,
      type: 'loading',
      title: 'ĐANG TẢI DỮ LIỆU...',
      msg: 'Hệ thống đang tải và nén dữ liệu vật phẩm (có thể nặng). Vui lòng KHÔNG đóng trang này cho đến khi nhận được tệp!',
      icon: HardDrive
    });

    try {
      const zip = new JSZip();
      zip.file("thongtin.txt", infoContent);
      
      // If mod has a link, save it to [Tên Mod].txt
      if (modItem && modItem.mod_link) {
        zip.file(`${itemName}.txt`, `LINK TẢI MOD: ${modItem.mod_link}`);
      }
      
      // If mod has a binary file, download and add it to zip
      if (modItem && modItem.file_url) {
        try {
          const response = await fetch(modItem.file_url);
          if (!response.ok) throw new Error('Network response was not ok');
          const blob = await response.blob();
          zip.file(modItem.file_name || `${itemName}_data.bin`, blob);
        } catch (fError) {
          console.error('Error fetching mod file:', fError);
          // Only add placeholder if the file should have been there but failed
          zip.file(`LỖI_TẢI_FILE.txt`, "Không thể tải file mod từ máy chủ. Vui lòng liên hệ Admin!");
        }
      }
      ...
      if (modItem) {
        ...
        await syncService.incrementModDownload(modItem.id);
      }
  ...
  const subscribeModal = !hasSubscribed && (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
      <div className="max-w-sm w-full bg-white/10 backdrop-blur-2xl p-8 rounded-3xl border border-white/20 text-center space-y-6 shadow-2xl zoom-in-95 duration-500">
          <div className="w-16 h-16 bg-red-600 rounded-2xl mx-auto flex items-center justify-center shadow-lg shadow-red-600/30">
              <Youtube size={32} className="text-white" />
          </div>
          <div>
              <h1 className="text-2xl font-black text-white uppercase tracking-tight">Subcrice #Hmai</h1>
              <p className="text-white/90 text-sm mt-2 font-medium">Bạn phải nhấn Đăng Ký Kênh để tắt thông báo và truy cập website.</p>
          </div>
          <button
            onClick={() => {
              window.open('https://youtube.com/@anhvuz35?si=rR_YPO9ijc8W35AY', '_blank');
              setTimeout(() => {
                setHasSubscribed(true);
              }, 2000);
            }}
            className="w-full py-3 mt-2 bg-red-600 hover:bg-red-700 rounded-xl font-bold transition-all shadow-lg shadow-red-600/40 text-sm flex items-center justify-center cursor-pointer text-white"
          >
            ...
            <div className="space-y-4 text-left">
                ...
                {authMode === 'register' && (
                  <div className="flex flex-col gap-2 pt-2 border-t border-gray-100 mt-2">
                    <label className="flex items-center text-sm text-gray-700 hover:text-black cursor-pointer select-none">
                      <input 
                        type="checkbox" 
                        checked={agreeToTerms} 
                        onChange={(e) => setAgreeToTerms(e.target.checked)} 
                        className="mr-2.5 w-4 h-4 rounded border-gray-200 bg-white text-orange-500 focus:ring-orange-500" 
                      />
                      <span>Tôi đồng ý với <button type="button" onClick={() => showModal('Điều Khoản Website', 'MDZPX cam kết bảo mật thông tin và cung cấp môi trường chơi game lành mạnh. Bằng việc đăng ký, bạn đồng ý với các quy định sau:\n\n1. Không sử dụng các phần mềm can thiệp trái phép vào game.\n2. Không lợi dụng lỗi hệ thống để trục lợi cá nhân.\n3. Bảo mật thông tin tài khoản, không chia sẻ cho người khác.\n4. Hệ thống có quyền khóa tài khoản nếu phát hiện gian lận hoặc vi phạm nghiêm trọng.\n5. Các giao dịch nạp/rút sẽ được xử lý trong vòng 24h.\n6. Một người dùng chỉ được sử dụng một tài khoản duy nhất.\n7. Admin có quyền quyết định cuối cùng trong các trường hợp tranh chấp.', Info)} className="text-orange-500 font-bold hover:underline ml-1">Điều khoản sử dụng</button></span>
                    </label>
                  </div>
                )}
      ...
      <main className="flex-1 h-full relative flex flex-col w-full max-w-full overflow-hidden">
        ...
        <div id="main-scroll-area" className="flex-1 overflow-y-auto custom-scrollbar flex flex-col">
            <div className="flex-1">
                ...
                {activeTab === 'present' && <PresentSection username={dbUserId || username} showModal={(title, msg, icon) => showModal(title, msg, icon)} />}
            ...
            <footer className="mt-auto p-8 border-t border-gray-200 text-center text-gray-500 text-sm bg-white">
                <p>&copy; 2026 MDZPX </p>
                <div className="flex justify-center flex-wrap gap-4 mt-2 text-xs font-medium">
                  <button onClick={() => showModal('Điều Khoản Website', '1. Không hack/cheat trong hệ thống.\n2. Tài khoản vi phạm sẽ bị khóa vĩnh viễn.\n3. Nạp tiền qua các kênh chính thống.\n4. Hỗ trợ 24/7 qua Telegram.', Info)} className="hover:text-orange-600 transition-colors uppercase tracking-widest">Điều khoản</button>
                  <button onClick={() => showModal('Chính Sách Bảo Mật', 'Hệ thống MDZPX cam kết không chia sẻ thông tin cá nhân của bạn cho bên thứ 3. Mọi dữ liệu giao dịch đều được mã hóa và bảo mật tuyệt đối.', ShieldCheck)} className="hover:text-orange-600 transition-colors uppercase tracking-widest">Bảo mật</button>
                </div>
            ...
            <div className="glass-morphism max-w-md w-full p-8 rounded-[40px] text-center space-y-6 border border-gray-200 shadow-2xl relative overflow-hidden">
              ...
              <div className="space-y-4">
                ...
                {modal.type === 'confirm' ? (
                  ...
                  <p className="text-gray-700 text-sm italic py-2 whitespace-pre-line text-left">{modal.msg}</p>
                )}
              </div>
              {modal.type !== 'loading' && (
