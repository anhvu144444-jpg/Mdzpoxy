...
export default function PullMoneySection({ handleExchange, pointsBalance, withdrawRequests, username }: { handleExchange: (item: string, price: string) => void, pointsBalance: number, withdrawRequests: any[], username: string }) {
  ...
  const handleWithdraw = () => {
    ...
    let itemName = '';
    const pointsCost = selectedAmount; 
  ...
  return (
    <section className="p-4 md:p-6 animate-in fade-in duration-300 max-w-7xl mx-auto space-y-6 text-gray-800">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            {amounts.map(amt => (
              <button 
                ...
                className={`py-3 px-2 rounded-lg text-xs font-bold text-center transition-all ${selectedAmount === amt ? 'bg-blue-500 text-white shadow-md' : 'bg-gray-50 text-gray-700 hover:bg-gray-100'}`}
              >
        ...
        {/* Selected Display */}
        {selectedAmount && (
           <div className="p-4 bg-gray-50 rounded-xl text-sm font-bold text-gray-800 border min-h-[50px] flex items-center">
             {selectedAmount.toLocaleString('vi-VN')}
           </div>
        )}
        {!selectedAmount && (
           <div className="p-4 bg-gray-50 rounded-xl text-sm font-bold text-gray-400 border min-h-[50px] flex items-center">
             Chưa chọn số tiền
           </div>
        )}
