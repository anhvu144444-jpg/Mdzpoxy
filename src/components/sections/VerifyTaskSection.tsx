    ...
    const verify = async () => {
        try {
            ...
            if (session && session.status === 'Đang làm') {
               ...
               if (
                   (actualUuid && targetUsername && actualUuid.toLowerCase() === targetUsername.toLowerCase()) || 
                   (session.user_id && targetUsername && session.user_id.toLowerCase() === targetUsername.toLowerCase())
               ) {
                   isValid = true;
               }
            }
  ...
  const handleConfirm = async () => {
    try {
        ...
        if (isValid && session) {
            ...
            const userProfile = await syncService.getProfile(session.user_id);
            if (userProfile) {
                const newPoints = (userProfile.points_balance || 0) + (session.reward || 0);
                await syncService.updateBalance(session.user_id, { points_balance: newPoints });

                if (setPointsBalance) {
                    setPointsBalance(newPoints);
                }

                // Award 10% commission of task points trọn đời (lifetime) to referrer
                await syncService.awardReferralCommission(session.user_id, session.reward || 0, session.task_name);
            }

            if (addActivityLog) {
                addActivityLog(session.user_id, `Hoàn thành nhiệm vụ ${session.task_name}`, `+${session.reward} Điểm`, 'deposit');
            }

            const maskedUser = session.user_id.length > 4 ? session.user_id.substring(0, 2) + '****' + session.user_id.substring(session.user_id.length - 2) : session.user_id.substring(0, 1) + '***';
