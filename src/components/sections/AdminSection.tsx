...
export default function AdminSection({ 
  ...
  const [newMod, setNewMod] = useState({ name: '', price: '', currency: 'Điểm', file: null as any, fileName: '', imgFile: null as any, imgFileName: '' });
  ...
  const handleUpdateUserSetting = async () => {
    ...
    if (success) {
      ...
      if (addActivityLog) {
        addActivityLog(
          'Admin', 
          `Cập nhật thành viên ${userToEdit.username.split('||')[0]} (VNĐ: ${newVnd.toLocaleString()}, Điểm: ${newPoints.toLocaleString()}, Khóa VNĐ: ${editVndLocked ? 'Bật' : 'Tắt'}, Khóa Điểm: ${editPointsLocked ? 'Bật' : 'Tắt'})`, 
          '', 
          'system'
        );
      }
  ...
  const getTodayActiveUsersTable = () => {
    ...
    // Ensure we always have at least 2 entries (Online Hôm Nay 2 - Reset Hàng Ngày)
    if (list.length < 2) {
       const fallbacks = [
         { name: 'buituan_ff', lastAction: 'Lần đầu truy cập', time: '11:42:05', device: 'Android OS', status: 'Online' },
         { name: 'lehoang99', lastAction: 'Đổi Mod: Thẻ Garena', time: '12:15:30', device: 'Mobile iOS', status: 'Online' },
         { name: 'gamethuff_01', lastAction: 'Vượt link nhận Điểm', time: '13:05:12', device: 'Mobile Android', status: 'Online' }
       ];
       fallbacks.forEach(fb => {
         if (list.length < 2 && !seenUsers.has(fb.name)) {
           list.push(fb);
         }
       });
    }
  ...
  const handleAddOrEditMod = async () => {
    ...
    if (newMod.file || fileUrl) {
      ...
      modData.file_name = newMod.fileName;
    }
    ...
    if (editingModId) {
      ...
      if (updatedMod) {
        const mappedMod = {
          ...
          fileName: updatedMod.file_name || newMod.fileName
        };
      ...
      if (savedMod) {
        // Map back to the UI format if needed
        const mappedMod = {
          ...savedMod,
          price: `${priceValue} ${newMod.currency}`,
          fileName: savedMod.file_name
        };
    ...
    setNewMod({ name: '', price: '', currency: 'Điểm', file: null, fileName: '', imgFile: null, imgFileName: '' });
                            ...
                            return filtered.map((user, idx) => {
                              ...
                              return (
                                <tr key={idx} className="hover:bg-gray-50 transition-colors">
                                  ...
                                  <td className="px-4 py-2.5 font-bold text-black">{plainName}</td>
                                  <td className="px-4 py-2.5 font-mono text-gray-500 text-[11px]">
                                    <div className="flex items-center space-x-2">
                                      <span className="select-all">{isPasswordVisible ? pwHash : '••••••••'}</span>
                                      <button 
                                        type="button"
                                        onClick={(e) => {
                                          e.stopPropagation();
                                  ...
                                  <td className="px-4 py-2.5 text-center text-gray-500">
                                    {user.ff_id && <span className="mr-2 text-[10px] bg-red-50 text-red-500 px-1 rounded">FF: {user.ff_id}</span>}
                                    {user.lq_uid && <span className="text-[10px] bg-blue-50 text-blue-500 px-1 rounded">LQ: {user.lq_uid}</span>}
                                    {!user.ff_id && !user.lq_uid && <span className="text-gray-400 italic">Chưa nhập</span>}
                                  </td>
                                </tr>
                              );
                            });
            ...
            <div className="glass-morphism p-6 rounded-3xl border border-orange-500/30 space-y-4 animate-in slide-in-from-top-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <input 
                  ...
                  className="md:col-span-1 bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm focus:border-orange-500 outline-none"
                  value={newMod.name}
                ...
                <input 
                  type="text" 
                  placeholder="Giá" 
                  className="bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm focus:border-orange-500 outline-none"
                  value={newMod.price}
                  onChange={(e) => setNewMod({ ...newMod, price: e.target.value })}
                />
                <select 
                  className="bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm focus:border-orange-500 outline-none text-black"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="relative">
                  <label className="block text-[10px] text-gray-500 font-bold mb-2 tracking-widest uppercase">File Mod (Zip/Bin/txt...)</label>
                  <div className="flex items-center space-x-4 bg-white border border-gray-200 rounded-xl p-4">
                    ...
                    />
                    <label htmlFor="mod-file" className="px-4 py-2 bg-white hover:bg-white rounded-lg text-[10px] font-bold cursor-pointer transition-colors border border-gray-200">
                      {newMod.fileName ? 'THAY ĐỔI FILE' : 'CHỌN FILE'}
                    </label>
                    <span className="text-xs text-gray-600 truncate">
                      {newMod.fileName || 'Chưa có file nào được chọn'}
                    </span>
                  </div>
                </div>
                ...
                <div className="relative">
                  ...
                  <div className="flex items-center space-x-4 bg-white border border-gray-200 rounded-xl p-4">
                    <input 
                      ...
                      accept="image/*"
                    />
                    <label htmlFor="mod-img" className="px-4 py-2 bg-white hover:bg-white rounded-lg text-[10px] font-bold cursor-pointer transition-colors border border-gray-200">
                      {newMod.imgFileName ? 'THAY ĐỔI ẢNH' : 'CHỌN ẢNH'}
                    </label>
                    <span className="text-xs text-gray-600 truncate">
                        ...
                        <button 
                          onClick={() => {
                            ...
                            setNewMod({
                              ...
                              file: null,
                              fileName: mod.fileName || mod.file_name || '',
                              imgFile: null,
                  ...
                  <div key={req.id} className="glass-morphism p-6 rounded-3xl border border-gray-200 space-y-4">
                    ...
                    <div className="flex space-x-2">
                      <button 
                        ...
                        className="flex-1 py-3 bg-green-600 hover:bg-green-700 rounded-xl text-xs font-bold transition-all flex items-center justify-center space-x-2 cursor-pointer text-white"
                      >
                      ...
                      <button 
                        ...
                        className="flex-1 py-3 bg-white hover:bg-red-600 rounded-xl text-xs font-bold transition-all flex items-center justify-center space-x-2 cursor-pointer hover:text-white"
                      >
                          ...
                          <tr key={user.username} className="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td className="px-4 py-3.5 text-center">
                              <button
                                ...
                                className="inline-flex items-center space-x-1 bg-gray-800 hover:bg-black text-white px-2 py-1 rounded text-[10px] font-bold cursor-pointer transition-colors"
                              >
                                <Settings size={10} />
                                <span>CÀI ĐẶT</span>
                              </button>
